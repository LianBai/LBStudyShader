Shader "LianBai/09-Specular Fragment BlinnPhong"
{
	properties
	{
...
#pragma fragment frag
		...
		//通过语义告诉系统: POSITION标明要把顶点值传进v、SV_POSITION标明函数返回值是剪裁空间下的顶点坐标
			...
			float4 frag(v2f f) : SV_Target
			{
				...
				//fixed3 viewDir = normalize(_WorldSpaceCameraPos.xyz - f.worldvertex.xyz); //_WorldSpaceCameraPos.xyz相机的位置，
				fixed3 viewDir = normalize(UnityWorldSpaceViewDir(f.worldvertex));
				fixed3 halfDir = normalize(viewDir + lightDir);
				fixed3 specular = _LightColor0.rgb * pow(max(0, dot(normalDir, halfDir)), _Gloss) * _SpecularColor.rgb;
